...
- Dashboard with quick links to start a new session
...
cd server
npm install
node migrate.js   # creates the User table
npm run dev
```
...
- Store full session history (question, answer, score) so users can see progress over time instead of just one question at a time
